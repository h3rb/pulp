...
public:
 ...
 void fromString( const char *s ) {
  ...
  if ( buf.length() > 255-2 ) buf.resize(255-2);
  name=buf;
  p=string_argument(p,&buf);
  i=atoi((char *)buf.c_str());
 }
...
public:
 ...
 string toString() {
  string r;
  EACH(first,Integer,i) r+=string("[")+i->toString()+string("] ");
  return r;
 }
 void fromString( const char *s ) {
  string buf;
...
// virtual int compare( Integer *A, Integer *B ) {
//  if ( A->i == B->i ) return 0;
//  if ( A->i > B->i ) return 1;
//  else return -1;
// }
// LL_SORTER(sort,Integers,Integer,compare);
 Integer *Sum() {
...
public: 
 ...
 Float *find( string s ) {
  FOREACH(Float,n) if ( string((char *)n->name) == s ) return n;
  return null;
 }
...
public:
 ...
 void Name( const char *s ) { name=string((char *)name); }
